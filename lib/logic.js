...
let generateAbbrlink = function (data) {
    ...
    if (!data.full_source.endsWith('.md')) {
        return;
    }
    ...
    if (data.layout == 'post') {
        ...
        if (!abbrlink || abbrlink == '0' || config.force) {
            ...
            if (opt_rep == 'hex') {
                let padLength = opt_alg == 'crc32' ? 8 : 4;
                abbrlink = abbrlink_value.toString(16).padStart(padLength, '0');
            } else {
                let padLength = opt_alg == 'crc32' ? 9 : 5;
                abbrlink = abbrlink_value.toString(10).padStart(padLength, '0');
            }
            data.abbrlink = abbrlink;
